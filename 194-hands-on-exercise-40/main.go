...
func main() {
	flavors := [...]string{"Almond Biscotti Café", "Banana Pudding ", "Balsamic Strawberry (GF)",
		...
		"Lavender Lemon Honey (GF)", "Lemon Bar", "Madagascar Vanilla (GF)", "Matcha (GF)",
		"Midnight Chocolate Sorbet (GF, V)", "Non-Dairy Chocolate Peanut Butter (GF, V)",
	...
	fmt.Println()
	fmt.Println()
	fmt.Printf("%T - is an Array of 31 Strings", flavors)
	fmt.Println()
	fmt.Println()
	fmt.Printf("There are %v flavors of ice cream here!", len(flavors))
}
